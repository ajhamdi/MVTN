...
from custom_dataset import ModelNet40, collate_fn, ShapeNetCore, ScanObjectNN  # , ModelNet40
...
parser.add_argument('--nb_views', type=int, help='number of views in the multi-view setup')
...
parser.add_argument('--exp_id', type=str, default='0001', help='pick ')
...
parser.add_argument('--return_points_sampled', dest='return_points_sampled',
                    action='store_true', help='reuturn 3d point clouds from the data loader sampled from the mesh  ')
...
parser.add_argument('--faces_per_pixel',  default=1, type=int,
                     help='max number of faces in every rendered pixel if `pc_rendering` is False ')
...
parser.add_argument('--background_color', '-bgc',  default="white", choices=["white", "random","black","red","green","blue", "custom"],
...
parser.add_argument('--mvtn_learning_rate', default=0.0001, type=float,
                    help='initial learning rate for view selector (default: 0.00001)')
parser.add_argument('--mvtn_weight_decay', default=0.01, type=float,
...
parser.add_argument('--mvtn_clip_grads_value', default=30.0, type=float,help='the clip value for L2 of gradeients of the MVTN ')
parser.add_argument('--shape_extractor', '-pnet',  default="PointNet", choices=["PointNet", "DGCNN", ],
...
parser.add_argument('--pretrained', dest='pretrained', action='store_true', help='use pre-trained CNN for the multi-view network ')
parser.add_argument('--depth', choices=[18, 34, 50, 101, 152],
                    type=int,  default=18, help='resnet depth (default: resnet18) for the CNN')
parser.add_argument('--view_reg', default=0.0, type=float,
                    help='use regulizer on the rendered views by dropout with this probablity   (default: 0.0)')
...
parser.add_argument("--viewgcn_phase", default="all", choices=["all", "first", "second", "third"],
...
parser.add_argument('--resume_mvtn', dest='resume_mvtn',
                    action='store_true', help='use a pretrained MVTN and freez during this training ( not the viewGCN)  ')
...
# view_selector = ViewSelector(setup["nb_views"], views_config=setup["views_config"],
...
# feature_extractor = FeatureExtractor(shape_features_size=setup["features_size"], views_config=setup["views_config"],
#                                      shape_extractor=setup["shape_extractor"], screatch_feature_extractor=setup["screatch_feature_extractor"]).cuda()
...
#               "optimizer": optimizer, "view_selector": view_selector, "mvtn_optimizer": mvtn_optimizer, "fe_optimizer": fe_optimizer,
#               "feature_extractor": feature_extractor}
models_bag = {"mvnetwork": mvnetwork,"optimizer": optimizer, "mvtn": mvtn, "mvtn_optimizer": mvtn_optimizer,"mvrenderer":mvrenderer}
...
def train(data_loader, models_bag, setup):
    ...
    for i, (targets, meshes, points) in enumerate(data_loader):
        ...
        if setup["is_learning_views"]:
            ...
            if setup["clip_grads"]:
                clip_grads_(models_bag["mvtn"].parameters(), setup["mvtn_clip_grads_value"])
            if setup["log_metrics"]:
                step = get_current_step(models_bag["mvtn_optimizer"])
                writer.add_scalar('Zoom/loss', loss.item(), step)
                writer.add_scalar('Zoom/MVT_vals', list(models_bag["mvtn"].parameters())[0].data[0, 0].item(), step)
                writer.add_scalar('Zoom/MVT_grads', np.sum(np.array([np.sum(x.grad.cpu().numpy() ** 2) for x in models_bag["mvtn"].parameters()])), step)
...
def evluate_late_fusion(data_loader, models_bag,  setup,):
    ...
    for i, (targets, meshes, points) in t:
        ...
        with torch.no_grad():
            ...
            point_features = models_bag["point_network"](points)
            if setup["features_type"] == "logits_trans":
                point_features = torch.cat((point_features[0].view(
                    c_batch_size, -1), point_features[2].view(c_batch_size, -1)), 1)
            elif setup["features_type"] == "post_max_trans":
                point_features = torch.cat((point_features[1].view(c_batch_size, -1), point_features[2].view(c_batch_size, -1)), 1)
            else:
                point_features =  point_features[features_order[setup["features_type"]]].view(c_batch_size, -1)
...
def visualize_retrieval_views(dtst, object_nbs,models_bag, setup):
...
# def analyze_rendered_views(dtst, object_nbs,models_bag, setup):
...
def view_gcn_exp(setup, models_bag, train_loader, val_loader, dset_val):
    ...
    if setup["resume_second"] or "test" in setup["run_mode"] :
        trainer.model.load(trainer.weights_dir,)
        if setup["is_learning_views"]:
...
if setup["mvnetwork"] == "mvcnn":
    ...
    #         saveables = {'epoch': epoch + 1,
    #                     'state_dict': models_bag["mvnetwork"].state_dict(),
    #                     # "view_selector": models_bag["mvtn"].state_dict(),
    #                     # "feature_extractor": models_bag["feature_extractor"].state_dict(),
    ...
    # #     print('\tCurrent best val acc: %.2f' % setup["best_acc"])
        ...
        for network in networks_list: 
            ...
            if network in ["DGCNN","PointNet"]:
                point_network.eval()
                load_point_ckpt(point_network,  setup["shape_extractor"],  ckpt_dir='./checkpoint')
                        ...
                        for factor in factor_list:
                            for axis in axis_list:
                                ...
                                if network not in ["PointNet","DGCNN"]:
                                    azim, elev, dist = models_bag["mvtn"](points, c_batch_size=c_batch_size)
                                    rendered_images, _ = models_bag["mvrenderer"](meshes, points,  azim=azim, elev=elev, dist=dist)                                    # save_grid(image_batch=rendered_images[0, ...],save_path=os.path.join(setup["results_dir"],"renderings","{}_{}.jpg".format(network,factor)), nrow=setup["nb_views"])
                                    outputs, _ = models_bag["mvnetwork"](rendered_images)
                                else: 
                                    chopped_pts = chopped_pts.transpose(1, 2).cuda()
                                    outputs = point_network(chopped_pts)[0].view(c_batch_size, -1)
